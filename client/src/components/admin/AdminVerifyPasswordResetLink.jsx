...
const AdminVerifyPasswordResetLink = () => {
  ...
  useEffect(() => {
    const verifyPasswordResetLink = async () => {
      try {
        const response = await userServices.adminVerifyPasswordResetLink(
          activationId
        );
        ...
        if (response.status === 200) {
          ...
          setTimeout(() => {
            navigate(
              `/admins/password/reset/verified/${response.data.redirectTo}`
            );
          }, 3000);
        } else {
...
export default AdminVerifyPasswordResetLink;
